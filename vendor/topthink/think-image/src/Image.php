...
class Image
{
     ...
     * @return $this
     */
    public function water($source, $locate = self::WATER_SOUTHEAST, $alpha = 100, $option = [])
    {
        if (!is_file($source)) {
        ...
        if(!empty($option)){
            ...
            $width = isset($option['width']) ? $option['width'] : $w;
            $height = isset($option['height']) ? $option['height'] : $h;
            ...
            if(!($w < $width && $h < $height)){
                ...
                //裁剪
                imagecopyresampled($img, $water, 0, 0, $x, $y, $width, $height, $w, $h);
                if (isset($option['radius'])) {
                    // 透明水印生成
                    $img = $this->radius_img($img, [$width, $height], $width / 2);
                }
     ...
     */
    private function radius_img($src_img, $wh, $radius = 15) {
        $w  = $wh[0];
        $h  = $wh[1];
