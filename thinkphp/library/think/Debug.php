...
class Debug
{
    ...
    public function save_log()
    {
        ...
        for ($i = $count - 1; $i > 0; $i--) {
            ...
            if ($i > 0) {
                $s = $key[$i - 1];
                $info[$e] = ['time' => $info[$key[$i]]];

                if (isset($info[$e]) && isset($info[$s])) {
                    $info[$e]['limit'] = $this->getRangeTime($s, $e);
                }

                if (isset($mem['mem']) && isset($mem['mem'][$e]) && isset($mem['mem'][$s])) {
                    $info[$e]['mem'] = $this->getRangeMem($s, $e);
                }

                if (isset($mem['peak']) && isset($mem['peak'][$e]) && isset($mem['peak'][$s])) {
                    $info[$e]['peak'] = $this->getRangeMem($s, $e);
                }
            }
        }
        ...
        $info['run'] = [
            'time' => microtime(true),
            'limit' => $this->getUseTime(),
            'mem' => $this->getUseMem(),
            'req' => $this->getThroughputRate()
        ];

        \think\facade\Log::record($info, 'debug');
        return true;
    }
}
